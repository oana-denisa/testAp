...
export class ClientService {
  ...
  protected convertDateFromClient(client: IClient): IClient {
    return Object.assign({}, client, {
      dataNastere: client.dataNastere?.isValid() ? client.dataNastere.toJSON() : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dataNastere = res.body.dataNastere ? dayjs(res.body.dataNastere) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((client: IClient) => {
        client.dataNastere = client.dataNastere ? dayjs(client.dataNastere) : undefined;
      });
    }
    return res;
  }
