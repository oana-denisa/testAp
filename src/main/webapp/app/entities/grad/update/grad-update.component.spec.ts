jest.mock('@angular/router');

import { ComponentFixture, TestBed } from '@angular/core/testing';
import { HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { FormBuilder } from '@angular/forms';
import { ActivatedRoute } from '@angular/router';
import { of, Subject } from 'rxjs';

import { GradService } from '../service/grad.service';
import { IGrad, Grad } from '../grad.model';

import { GradUpdateComponent } from './grad-update.component';

describe('Component Tests', () => {
  describe('Grad Management Update Component', () => {
    let comp: GradUpdateComponent;
    let fixture: ComponentFixture<GradUpdateComponent>;
    let activatedRoute: ActivatedRoute;
    let gradService: GradService;

    beforeEach(() => {
      TestBed.configureTestingModule({
        imports: [HttpClientTestingModule],
        declarations: [GradUpdateComponent],
        providers: [FormBuilder, ActivatedRoute],
      })
        .overrideTemplate(GradUpdateComponent, '')
        .compileComponents();

      fixture = TestBed.createComponent(GradUpdateComponent);
      activatedRoute = TestBed.inject(ActivatedRoute);
      gradService = TestBed.inject(GradService);

      comp = fixture.componentInstance;
    });

    describe('ngOnInit', () => {
      it('Should update editForm', () => {
        const grad: IGrad = { id: 456 };

        activatedRoute.data = of({ grad });
        comp.ngOnInit();

        expect(comp.editForm.value).toEqual(expect.objectContaining(grad));
      });
    });

    describe('save', () => {
      it('Should call update service on save for existing entity', () => {
        // GIVEN
        const saveSubject = new Subject();
        const grad = { id: 123 };
        spyOn(gradService, 'update').and.returnValue(saveSubject);
        spyOn(comp, 'previousState');
        activatedRoute.data = of({ grad });
        comp.ngOnInit();

        // WHEN
        comp.save();
        expect(comp.isSaving).toEqual(true);
        saveSubject.next(new HttpResponse({ body: grad }));
        saveSubject.complete();

        // THEN
        expect(comp.previousState).toHaveBeenCalled();
        expect(gradService.update).toHaveBeenCalledWith(grad);
        expect(comp.isSaving).toEqual(false);
      });

      it('Should call create service on save for new entity', () => {
        // GIVEN
        const saveSubject = new Subject();
        const grad = new Grad();
        spyOn(gradService, 'create').and.returnValue(saveSubject);
        spyOn(comp, 'previousState');
        activatedRoute.data = of({ grad });
        comp.ngOnInit();

        // WHEN
        comp.save();
        expect(comp.isSaving).toEqual(true);
        saveSubject.next(new HttpResponse({ body: grad }));
        saveSubject.complete();

        // THEN
        expect(gradService.create).toHaveBeenCalledWith(grad);
        expect(comp.isSaving).toEqual(false);
        expect(comp.previousState).toHaveBeenCalled();
      });

      it('Should set isSaving to false on error', () => {
        // GIVEN
        const saveSubject = new Subject();
        const grad = { id: 123 };
        spyOn(gradService, 'update').and.returnValue(saveSubject);
        spyOn(comp, 'previousState');
        activatedRoute.data = of({ grad });
        comp.ngOnInit();

        // WHEN
        comp.save();
        expect(comp.isSaving).toEqual(true);
        saveSubject.error('This is an error!');

        // THEN
        expect(gradService.update).toHaveBeenCalledWith(grad);
        expect(comp.isSaving).toEqual(false);
        expect(comp.previousState).not.toHaveBeenCalled();
      });
    });
  });
});
